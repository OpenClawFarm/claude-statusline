...
**TPS (7)** — Calculates `output_tokens / streaming_time` from JSONL, excluding tool execution time. Filters: ≥100 tokens (≥10 during cold start), ≥0.3s, 10–500 tps range. Sliding window median over last 10 samples. Aggregates across all active sessions.

**RTT (8)** — Pings `api.anthropic.com` every 5s (single ICMP packet, 2s timeout). Sliding window median over last 20 rounds. Falls back to `curl` TTFB if ICMP is blocked.
